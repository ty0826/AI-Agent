...
graph.invoke({"value": []}, config)  ###首次执行interrupt中断
graph.invoke(Command(resume="Alice"), config)  ###首次执行输入的值{"value": [hello Alice，done]}
res = graph.invoke(Command(resume="22"), config) ###多层断点
print(res)
