...
# 使用 Redis 持久化记忆：
#   - RedisSaver  : 短期记忆（同一 thread_id 的多轮对话历史 / checkpoint）
#   - RedisStore  : 长期记忆（用户资料 profile、待办 todo、更新说明 instructions）
import redis
from config.cache_conf import REDIS_HOST, REDIS_PORT, REDIS_DB

REDIS_URL = f"redis://{REDIS_HOST}:{REDIS_PORT}/{REDIS_DB}"

# checkpointer 负责把每个会话(thread_id)的对话历史写入 redis，实现"记忆"
memorySaver = RedisSaver(redis_url=REDIS_URL)
memorySaver.setup()  # 首次创建 redis 索引（幂等，可重复调用）

# store 负责按 user_id 跨会话存储用户长期记忆
memoryStore = RedisStore(redis.Redis(host=REDIS_HOST, port=REDIS_PORT, db=REDIS_DB))
memoryStore.setup()
